...
#include "DebugServer2/Architecture/MIPS/SoftwareSingleStep.h"
...
#endif
  return static_cast<uint32_t>(state.gp.regs[reg]);
}

ErrorCode PrepareMIPSSoftwareSingleStep(Process *process, uint64_t pc,
                                        CPUState const &state, bool is64bit,
                                        uint64_t &nextPC, uint32_t &nextPCSize,
                                        uint64_t &branchPC,
  ...
  case kOpcodeSpecial:
    ...
    if (func == kFuncJR || func == kFuncJALR) {
      isBranch = true;
      takeBranch = true;
      targetAddr = GetRegValue(state, rs, is64bit);
      DS2LOG(Debug, "MIPS %s to reg r%d = %#llx at PC=%#llx",
  ...
  case kOpcodeJAL:
    ...
    isBranch = true;
    takeBranch = true;
    // Target is (PC+4)[31:28] || target || 00
    targetAddr = ((pc + 4) & (is64bit ? 0xFFFFFFFFF0000000ULL : 0xF0000000)) |
                 (target << 2);
    DS2LOG(Debug, "MIPS %s to %#llx at PC=%#llx", opcode == kOpcodeJ ? "J" : "JAL",
           (unsigned long long)targetAddr, (unsigned long long)pc);
    break;
  ...
  case kOpcodeBeql:
    // Branch if equal
    isBranch = true;
    takeBranch = (GetRegValue(state, rs, is64bit) ==
                  GetRegValue(state, rt, is64bit));
    targetAddr = pc + 4 + (SignExtend(imm, 16) << 2);
    DS2LOG(Debug, "MIPS BEQ r%d(=%#llx), r%d(=%#llx) -> %s at PC=%#llx", rs,
           (unsigned long long)GetRegValue(state, rs, is64bit), rt,
           (unsigned long long)GetRegValue(state, rt, is64bit),
  ...
  case kOpcodeBnel:
    ...
    takeBranch = (GetRegValue(state, rs, is64bit) !=
                  GetRegValue(state, rt, is64bit));
    targetAddr = pc + 4 + (SignExtend(imm, 16) << 2);
    DS2LOG(Debug, "MIPS BNE r%d, r%d -> %s at PC=%#llx", rs, rt,
           takeBranch ? "taken" : "not taken", (unsigned long long)pc);
    break;
  ...
  case kOpcodeBlezl:
    ...
    isBranch = true;
    takeBranch =
        (static_cast<int64_t>(GetRegValue(state, rs, is64bit)) <= 0);
    targetAddr = pc + 4 + (SignExtend(imm, 16) << 2);
    DS2LOG(Debug, "MIPS BLEZ r%d -> %s at PC=%#llx", rs,
           takeBranch ? "taken" : "not taken", (unsigned long long)pc);
    break;
  ...
  case kOpcodeBgtzl:
    ...
    isBranch = true;
    takeBranch = (static_cast<int64_t>(GetRegValue(state, rs, is64bit)) > 0);
    targetAddr = pc + 4 + (SignExtend(imm, 16) << 2);
    DS2LOG(Debug, "MIPS BGTZ r%d -> %s at PC=%#llx", rs,
           takeBranch ? "taken" : "not taken", (unsigned long long)pc);
    break;
  ...
  case kOpcodeRegimm:
    ...
    case kRegimmBltzl:
      takeBranch = (static_cast<int64_t>(GetRegValue(state, rs, is64bit)) < 0);
      DS2LOG(Debug, "MIPS BLTZ r%d -> %s at PC=%#llx", rs,
             takeBranch ? "taken" : "not taken", (unsigned long long)pc);
      break;
    case kRegimmBGEZ:
    case kRegimmBgezl:
      takeBranch =
          (static_cast<int64_t>(GetRegValue(state, rs, is64bit)) >= 0);
      DS2LOG(Debug, "MIPS BGEZ r%d -> %s at PC=%#llx", rs,
             takeBranch ? "taken" : "not taken", (unsigned long long)pc);
      break;
    case kRegimmBLTZAL:
      takeBranch = (static_cast<int64_t>(GetRegValue(state, rs, is64bit)) < 0);
      DS2LOG(Debug, "MIPS BLTZAL r%d -> %s at PC=%#llx", rs,
             takeBranch ? "taken" : "not taken", (unsigned long long)pc);
      break;
    case kRegimmBGEZAL:
      takeBranch =
          (static_cast<int64_t>(GetRegValue(state, rs, is64bit)) >= 0);
      DS2LOG(Debug, "MIPS BGEZAL r%d -> %s at PC=%#llx", rs,
             takeBranch ? "taken" : "not taken", (unsigned long long)pc);
      break;
    default:
      isBranch = false;
  ...
  case kOpcodeCop1: {
    ...
    if (fmt == 0x08) { // BC1
      ...
      targetAddr = pc + 4 + (SignExtend(imm, 16) << 2);
      DS2LOG(Debug, "MIPS BC1 cc=%d, tf=%d -> %s at PC=%#llx", cc, tf,
             takeBranch ? "taken" : "not taken", (unsigned long long)pc);
    }
    break;
  }
...
ErrorCode PrepareMicroMIPSSoftwareSingleStep(
    Process *process, uint64_t pc, CPUState const &state, bool is64bit,
...
ErrorCode PrepareSoftwareSingleStep(Process *process,
  ...
  if (pc & 0x1) {
    ...
    CHK(PrepareMIPS16SoftwareSingleStep(process, pc, state, is64bit, nextPC,
                                        nextPCSize, branchPC, branchPCSize));
  } else {
    // Normal MIPS mode
    CHK(PrepareMIPSSoftwareSingleStep(process, pc, state, is64bit, nextPC,
                                      nextPCSize, branchPC, branchPCSize));
  }

  DS2LOG(Debug,
         "PC=%#llx, branchPC=%#llx[size=%d] nextPC=%#llx[size=%d]",
