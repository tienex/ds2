...
//
// GNU/Hurd Process - adapted from Darwin's Mach-based implementation
//
...
ErrorCode Process::attach(int waitStatus) {
  ...
  if (_flags & kFlagAttachedProcess) {
    // Enumerate all the tasks and create a Thread object for every entry
    // GNU/Hurd: Thread enumeration via Mach
    bool keep_going = true;

    while (keep_going) {
      keep_going = false;
      // TODO: Implement thread enumeration via Mach task_threads()
    }
  }
...
ds2::Host::Hurd::Mach &Process::mach() const {
...
ErrorCode Process::updateInfo() {
  // GNU/Hurd: Get process info
  // Unlike Darwin which has libproc, Hurd uses proc server
