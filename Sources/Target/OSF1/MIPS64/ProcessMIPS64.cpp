...
//
// OSF/1 MIPS64 Process - Syscall injection for memory allocation
//
...
// OSF/1 syscall numbers for MIPS64 (N64 ABI)
static const int OSF1_SYS_mmap = 71;
static const int OSF1_SYS_munmap = 73;

//
// MIPS64 syscall injection code - N64 ABI
// N64: a0-a7 for first 8 args, syscall number in v0
//

// mmap syscall for MIPS64 N64
// All 6 args fit in registers
static uint32_t const gMIPS64N64MmapCode[] = {
...
// munmap syscall for MIPS64 N64
static uint32_t const gMIPS64N64MunmapCode[] = {
    0x24020000,  // li   $v0, 0      # syscall number (patched)
...
static void MIPSPrepareMmapCode(size_t size, int protection,
                                ByteVector &codestr) {
  InitCodeVector(codestr, gMIPS64N64MmapCode);
  uint32_t *code = reinterpret_cast<uint32_t *>(&codestr[0]);

  MIPSSetLIImmediate(code + 0, OSF1_SYS_mmap & 0xffff);        // syscall number
...
static void MIPSPrepareMunmapCode(uint64_t address, size_t size,
                                  ByteVector &codestr) {
  InitCodeVector(codestr, gMIPS64N64MunmapCode);
  uint32_t *code = reinterpret_cast<uint32_t *>(&codestr[0]);
...
ErrorCode Process::allocateMemory(size_t size, uint32_t protection,
  ...
  // Save current PC
  uint64_t pc = state.mips64.special.pc;
  ...
  *address = state.mips64.gp.v0;

  // Restore PC
  state.mips64.special.pc = pc;
