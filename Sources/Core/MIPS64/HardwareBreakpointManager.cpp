...
int HardwareBreakpointManager::hit(Target::Thread *thread, Site &site) {
  // MIPS64 hardware breakpoints are not currently supported
  return -1;
};
...
ErrorCode HardwareBreakpointManager::isValid(Address const &address,
  ...
  DS2LOG(Debug, "Trying to set hardware breakpoint on MIPS64");
  // MIPS64 hardware breakpoints are not currently supported
  return kErrorUnsupported;
}
...
size_t HardwareBreakpointManager::chooseBreakpointSize() const {
  DS2BUG(
      "Choosing a hardware breakpoint size on MIPS64 is an unsupported operation");
}
} // namespace ds2
