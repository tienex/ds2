...
ErrorCode SoftwareBreakpointManager::add(Address const &address,
  ...
  if (size != 2 && size != 4) {
    ...
    // Bit 0 clear means normal MIPS (32-bit)
...
int SoftwareBreakpointManager::hit(Target::Thread *thread, Site &site) {
  ...
  // On MIPS, when a breakpoint is hit, the PC points to the break instruction
  // (not past it like on x86). No adjustment needed on POSIX.
#if defined(OS_WIN32)
  // On Windows, we may need to adjust PC back
  // This depends on the Windows MIPS debugger behavior
...
void SoftwareBreakpointManager::getOpcode(uint32_t type,
  ...
  // MIPS breakpoint instructions:
  // Normal MIPS (4 bytes): break instruction with code
  ...
  case 4:
    // 4-byte breakpoint for normal MIPS
    // 0x0005000d (break 5)
  ...
  default:
    DS2LOG(Error, "invalid MIPS breakpoint type %d (expected 2 or 4)", type);
...
ErrorCode SoftwareBreakpointManager::isValid(Address const &address,
  ...
  // MIPS supports 2-byte (MIPS16/microMIPS) and 4-byte (normal) breakpoints
  if (size != 2 && size != 4) {
    DS2LOG(Debug, "Received unsupported MIPS breakpoint size %zu (expected 2 or 4)",
           size);
...
size_t SoftwareBreakpointManager::chooseBreakpointSize() const {
  // Default to 4-byte MIPS breakpoint
