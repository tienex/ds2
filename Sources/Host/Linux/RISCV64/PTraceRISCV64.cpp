...
struct user_fpregs_struct {
  uint64_t f[32];   // FP registers (64-bit)
...
ErrorCode PTrace::readGPRegisters(ProcessThreadId const &ptid,
  ...
  values.push_back(Architecture::GPRegisterValue{
      0, Architecture::GPRegisterValue::k64bits, static_cast<uint64_t>(0)});

  // Map from user_regs_struct to x1-x31
  uint64_t regs_ordered[31] = {
  ...
  for (int i = 0; i < 31; i++) {
    values.push_back(Architecture::GPRegisterValue{
        static_cast<uint32_t>(i + 1), Architecture::GPRegisterValue::k64bits,
        regs_ordered[i]});
  }

  // Add PC (register 32)
  values.push_back(Architecture::GPRegisterValue{
      32, Architecture::GPRegisterValue::k64bits, gprs.pc});
