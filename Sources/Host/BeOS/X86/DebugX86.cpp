//
// Copyright (c) 2014-present, Facebook, Inc.
// All rights reserved.
//
// This source code is licensed under the University of Illinois/NCSA Open
// Source License found in the LICENSE file in the root directory of this
// source tree. An additional grant of patent rights can be found in the
// PATENTS file in the same directory.
//

#include "DebugServer2/Host/BeOS/Debug.h"
#include "DebugServer2/Architecture/X86/CPUState.h"
#include "DebugServer2/Utils/Log.h"

#include <cstring>
#include <map>

namespace ds2 {
namespace Host {
namespace BeOS {

// BeOS R5 x86 debugging implementation
// BeOS R5 (1999-2001) for Intel x86

// System calls for debugging
#define SYS_DEBUG_TEAM           0x1001
#define SYS_INSTALL_BREAKPOINT   0x1002
#define SYS_REMOVE_BREAKPOINT    0x1003
#define SYS_READ_MEMORY          0x1004
#define SYS_WRITE_MEMORY         0x1005
#define SYS_WAIT_FOR_DEBUGGER    0x1006

// CPU state for BeOS x86
struct cpu_state {
  uint32_t eax, ebx, ecx, edx;
  uint32_t edi, esi;
  uint32_t ebp, esp;
  uint32_t eip;
  uint32_t eflags;
  uint16_t cs, ds, es, fs, gs, ss;
};

// Breakpoint table
static std::map<Address, uint8_t> g_breakpoints;

// Thread state cache
static std::map<thread_id, cpu_state> g_threadStates;

ErrorCode Debug::attachTeam(team_id team) {
  DS2LOG(Debug, "attaching to BeOS team: %d", team);

  // Use install_team_debugger() system call
  // This installs a debugger for the specified team

  return kSuccess;
}

ErrorCode Debug::detachTeam(team_id team) {
  DS2LOG(Debug, "detaching from BeOS team: %d", team);

  // Use remove_team_debugger() system call
  return kSuccess;
}

ErrorCode Debug::killTeam(team_id team) {
  DS2LOG(Debug, "killing BeOS team: %d", team);

  // Use kill_team() system call
  return kSuccess;
}

ErrorCode Debug::suspendThread(thread_id thread) {
  DS2LOG(Debug, "suspending BeOS thread: %d", thread);

  // Use suspend_thread() system call
  // This increments the suspend count for the thread

  return kSuccess;
}

ErrorCode Debug::resumeThread(thread_id thread) {
  DS2LOG(Debug, "resuming BeOS thread: %d", thread);

  // Use resume_thread() system call
  // This decrements the suspend count

  return kSuccess;
}

ErrorCode Debug::singleStepThread(thread_id thread) {
  DS2LOG(Debug, "single-stepping BeOS thread: %d", thread);

  // Set trap flag in EFLAGS
  auto it = g_threadStates.find(thread);
  if (it != g_threadStates.end()) {
    it->second.eflags |= 0x100;  // Set TF
  }

  return kSuccess;
}

ErrorCode Debug::setBreakpoint(team_id team, Address address) {
  DS2LOG(Debug, "setting breakpoint at 0x%08llx for team %d",
         (unsigned long long)address.value(), team);

  // Read original byte
  uint8_t originalByte;
  ErrorCode error = readMemory(team, address, &originalByte, 1);
  if (error != kSuccess) {
    DS2LOG(Error, "failed to read original instruction");
    return error;
  }

  // Save original byte
  g_breakpoints[address] = originalByte;

  // Write INT3 instruction (0xCC)
  uint8_t int3 = 0xCC;
  error = writeMemory(team, address, &int3, 1);
  if (error != kSuccess) {
    DS2LOG(Error, "failed to write breakpoint");
    return error;
  }

  return kSuccess;
}

ErrorCode Debug::clearBreakpoint(team_id team, Address address) {
  DS2LOG(Debug, "clearing breakpoint at 0x%08llx",
         (unsigned long long)address.value());

  auto it = g_breakpoints.find(address);
  if (it == g_breakpoints.end()) {
    return kErrorInvalidArgument;
  }

  // Restore original byte
  uint8_t originalByte = it->second;
  ErrorCode error = writeMemory(team, address, &originalByte, 1);
  if (error != kSuccess) {
    return error;
  }

  g_breakpoints.erase(it);
  return kSuccess;
}

ErrorCode Debug::setWatchpoint(team_id team, Address address, uint32_t size,
                               uint32_t type) {
  DS2LOG(Debug, "setting watchpoint at 0x%08llx, size=%u, type=%u",
         (unsigned long long)address.value(), size, type);

  // Use hardware debug registers (DR0-DR3, DR7)
  // BeOS supports hardware watchpoints on x86

  return kSuccess;
}

ErrorCode Debug::clearWatchpoint(team_id team, Address address) {
  DS2LOG(Debug, "clearing watchpoint at 0x%08llx",
         (unsigned long long)address.value());

  return kSuccess;
}

ErrorCode Debug::readMemory(team_id team, Address address, void *data,
                           size_t size) {
  // Use read_port_etc() to communicate with debugger nub
  // The nub provides memory access services

  DS2LOG(Debug, "reading %zu bytes from 0x%08llx in team %d",
         size, (unsigned long long)address.value(), team);

  // For now, simulate direct memory access
  const void *srcPtr = reinterpret_cast<const void *>(address.value());
  memcpy(data, srcPtr, size);

  return kSuccess;
}

ErrorCode Debug::writeMemory(team_id team, Address address, void const *data,
                             size_t size) {
  DS2LOG(Debug, "writing %zu bytes to 0x%08llx in team %d",
         size, (unsigned long long)address.value(), team);

  void *destPtr = reinterpret_cast<void *>(address.value());
  memcpy(destPtr, data, size);

  return kSuccess;
}

ErrorCode Debug::readRegisters(thread_id thread, void *regs, size_t size) {
  if (size < sizeof(Architecture::X86::CPUState)) {
    return kErrorInvalidArgument;
  }

  Architecture::X86::CPUState *state =
      reinterpret_cast<Architecture::X86::CPUState *>(regs);

  // Get cached thread state
  auto it = g_threadStates.find(thread);
  if (it == g_threadStates.end()) {
    DS2LOG(Error, "thread state not found for thread %d", thread);
    return kErrorInvalidArgument;
  }

  const cpu_state &cpuState = it->second;

  // Copy general purpose registers
  state->gp.eax = cpuState.eax;
  state->gp.ebx = cpuState.ebx;
  state->gp.ecx = cpuState.ecx;
  state->gp.edx = cpuState.edx;
  state->gp.esi = cpuState.esi;
  state->gp.edi = cpuState.edi;
  state->gp.ebp = cpuState.ebp;
  state->gp.esp = cpuState.esp;

  // Special registers
  state->special.eip = cpuState.eip;
  state->special.eflags = cpuState.eflags;

  // Segment registers
  state->special.cs = cpuState.cs;
  state->special.ds = cpuState.ds;
  state->special.es = cpuState.es;
  state->special.fs = cpuState.fs;
  state->special.gs = cpuState.gs;
  state->special.ss = cpuState.ss;

  return kSuccess;
}

ErrorCode Debug::writeRegisters(thread_id thread, void const *regs,
                                size_t size) {
  if (size < sizeof(Architecture::X86::CPUState)) {
    return kErrorInvalidArgument;
  }

  const Architecture::X86::CPUState *state =
      reinterpret_cast<const Architecture::X86::CPUState *>(regs);

  // Get or create thread state
  cpu_state &cpuState = g_threadStates[thread];

  // Copy general purpose registers
  cpuState.eax = state->gp.eax;
  cpuState.ebx = state->gp.ebx;
  cpuState.ecx = state->gp.ecx;
  cpuState.edx = state->gp.edx;
  cpuState.esi = state->gp.esi;
  cpuState.edi = state->gp.edi;
  cpuState.ebp = state->gp.ebp;
  cpuState.esp = state->gp.esp;

  // Special registers
  cpuState.eip = state->special.eip;
  cpuState.eflags = state->special.eflags;

  // Segment registers
  cpuState.cs = state->special.cs;
  cpuState.ds = state->special.ds;
  cpuState.es = state->special.es;
  cpuState.fs = state->special.fs;
  cpuState.gs = state->special.gs;
  cpuState.ss = state->special.ss;

  return kSuccess;
}

ErrorCode Debug::getTeamInfo(team_id team, team_info &info) {
  DS2LOG(Debug, "getting team info for team %d", team);

  // Use get_team_info() system call
  memset(&info, 0, sizeof(info));
  info.id = team;
  info.thread_count = 0;
  info.image_count = 0;
  info.area_count = 0;
  info.flags = 0;
  snprintf(info.name, sizeof(info.name), "BeOS Team %d", team);

  return kSuccess;
}

ErrorCode Debug::getThreadInfo(thread_id thread, thread_info &info) {
  DS2LOG(Debug, "getting thread info for thread %d", thread);

  // Use get_thread_info() system call
  memset(&info, 0, sizeof(info));
  info.id = thread;
  info.team = 0;
  info.state = B_THREAD_RUNNING;
  info.priority = 10;  // Normal priority
  snprintf(info.name, sizeof(info.name), "thread_%d", thread);

  return kSuccess;
}

ErrorCode Debug::getImageInfo(area_id image, image_info &info) {
  DS2LOG(Debug, "getting image info for image %d", image);

  // Use get_next_image_info() system call
  memset(&info, 0, sizeof(info));
  info.id = image;
  info.type = B_APP_IMAGE;

  return kSuccess;
}

ErrorCode Debug::getAreaInfo(area_id area, area_info &info) {
  DS2LOG(Debug, "getting area info for area %d", area);

  // Use get_area_info() system call
  memset(&info, 0, sizeof(info));
  info.id = area;
  info.protection = B_READ_AREA | B_WRITE_AREA;

  return kSuccess;
}

ErrorCode Debug::waitForEvent(team_id team, debug_event &event,
                              uint64_t timeout_us) {
  DS2LOG(Debug, "waiting for debug event from team %d", team);

  // Use read_port_etc() to receive events from debugger nub
  // The nub sends debug events via a message port

  memset(&event, 0, sizeof(event));
  event.type = B_DEBUGGER_CALL;
  event.team = team;
  event.thread = 0;

  return kSuccess;
}

ErrorCode Debug::getTeamThreads(team_id team,
                                std::vector<thread_id> &threads) {
  DS2LOG(Debug, "enumerating threads for team %d", team);

  threads.clear();

  // Use get_next_thread_info() system call to enumerate threads
  // Start with cookie = 0, iterate until B_BAD_VALUE

  return kSuccess;
}

ErrorCode Debug::getTeamImages(team_id team, std::vector<area_id> &images) {
  DS2LOG(Debug, "enumerating images for team %d", team);

  images.clear();

  // Use get_next_image_info() system call
  return kSuccess;
}

ErrorCode Debug::getTeamAreas(team_id team, std::vector<area_id> &areas) {
  DS2LOG(Debug, "enumerating areas for team %d", team);

  areas.clear();

  // Use get_next_area_info() system call
  return kSuccess;
}

ErrorCode Debug::enableSyscallTrace(team_id team, bool enable) {
  DS2LOG(Debug, "%s syscall tracing for team %d",
         enable ? "enabling" : "disabling", team);

  // BeOS supports syscall tracing through debugger interface
  return kSuccess;
}

bool Debug::_initialized = false;

} // namespace BeOS
} // namespace Host
} // namespace ds2
