...
ErrorCode Debug::setBreakpoint(pid_t pid, Address addr) { uint8_t o; readMemory(pid,addr,&o,1); g_bp[addr]=o; uint8_t i=0xCC; return writeMemory(pid,addr,&i,1); }
ErrorCode Debug::clearBreakpoint(pid_t pid, Address addr) { auto it=g_bp.find(addr); if(it==g_bp.end()) return kErrorInvalidArgument; return writeMemory(pid,addr,&it->second,1); }
