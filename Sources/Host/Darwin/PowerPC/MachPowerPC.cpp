...
ErrorCode Mach::readCPUState(ProcessThreadId const &ptid,
  ...
  // Read PowerPC thread state
  ppc_thread_state_t gprs;
  mach_msg_type_number_t count = PPC_THREAD_STATE_COUNT;

  kern_return_t kr = thread_get_state(thread, PPC_THREAD_STATE,
  ...
  for (size_t n = 0; n < 32; n++) {
    state.ppc.gp.regs[n] = gprs.r[n];
  }

  // Copy special registers
  state.ppc.pc = gprs.srr0;  // PC is in SRR0
  state.ppc.msr = gprs.srr1; // MSR is in SRR1
  ...
  if (kr == KERN_SUCCESS) {
    for (size_t n = 0; n < 32; n++) {
      state.ppc.fp.regs[n] = fprs.fpregs[n];
    }
  ...
  if (kr == KERN_SUCCESS) {
    for (size_t n = 0; n < 32; n++) {
      for (size_t i = 0; i < 4; i++) {
        state.ppc.vr[n].v[i] = vrs.save_vr[n][i];
      }
    }
    state.ppc.vscr = vrs.save_vscr[3]; // VSCR is in word 3
    state.ppc.vrsave = vrs.save_vrsave;
  }
...
ErrorCode Mach::writeCPUState(ProcessThreadId const &ptid,
  ...
  // Prepare PowerPC thread state
  ppc_thread_state_t gprs;
  mach_msg_type_number_t count = PPC_THREAD_STATE_COUNT;

  // Copy general-purpose registers
  for (size_t n = 0; n < 32; n++) {
    gprs.r[n] = state.ppc.gp.regs[n];
  }

  // Copy special registers
  gprs.srr0 = state.ppc.pc;
  gprs.srr1 = state.ppc.msr;
  gprs.lr = state.ppc.lr;
  gprs.ctr = state.ppc.ctr;
  gprs.xer = state.ppc.xer;
  gprs.cr = state.ppc.cr;

  kern_return_t kr = thread_set_state(thread, PPC_THREAD_STATE,
                                      (thread_state_t)&gprs, count);
  if (kr != KERN_SUCCESS)
    return Platform::TranslateError(kr);
  ...
  for (size_t n = 0; n < 32; n++) {
    fprs.fpregs[n] = state.ppc.fp.regs[n];
  }
  ...
  for (size_t n = 0; n < 32; n++) {
    for (size_t i = 0; i < 4; i++) {
      vrs.save_vr[n][i] = state.ppc.vr[n].v[i];
    }
  }
  vrs.save_vscr[3] = state.ppc.vscr; // VSCR is in word 3
  vrs.save_vrsave = state.ppc.vrsave;
