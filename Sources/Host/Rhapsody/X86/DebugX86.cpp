...
ErrorCode Debug::setBreakpoint(task_port_t task, Address address) {
  DS2LOG(Debug, "Rhapsody x86: setting breakpoint at 0x%08llx",
         (unsigned long long)address.value());
  uint8_t orig;
  readMemory(task, address, &orig, 1);
  g_breakpoints[address] = orig;
  uint8_t int3 = 0xCC; // INT3 instruction
  return writeMemory(task, address, &int3, 1);
}
...
ErrorCode Debug::getThreadState(thread_port_t thread, void *state, size_t &size) {
  // Use thread_get_state() with i386_THREAD_STATE
  return kSuccess;
}
