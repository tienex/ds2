...
static std::map<Address, uint16_t> g_bp;
...
ErrorCode Debug::setBreakpoint(pid_t pid, Address addr) {
  uint16_t orig; readMemory(pid, addr, &orig, 2); g_bp[addr] = orig;
  uint16_t trapa = 0xC320; // TRAPA #32
  return writeMemory(pid, addr, &trapa, 2);
}
ErrorCode Debug::clearBreakpoint(pid_t pid, Address addr) {
  auto it = g_bp.find(addr); if (it == g_bp.end()) return kErrorInvalidArgument;
  return writeMemory(pid, addr, &it->second, 2);
}
