...
static std::map<Address, uint8_t> g_bp;
...
ErrorCode Debug::setBreakpoint(task_port_t task, Address addr) { uint8_t o; readMemory(task,addr,&o,1); g_bp[addr]=o; uint8_t i=0xCC; return writeMemory(task,addr,&i,1); }
ErrorCode Debug::clearBreakpoint(task_port_t task, Address addr) { auto it=g_bp.find(addr); if(it==g_bp.end()) return kErrorInvalidArgument; return writeMemory(task,addr,&it->second,1); }
