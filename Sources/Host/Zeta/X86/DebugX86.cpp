...
// Zeta x86 debugging implementation
// Zeta (2001-2007) - commercial continuation of BeOS by yellowTAB

// Zeta extends BeOS with additional debugging features
// - Enhanced kernel debugging interface
// - Improved thread synchronization debugging
// - Memory leak detection support
// - Enhanced signal handling

// CPU state for Zeta x86
struct cpu_state {
  ...
  // Zeta extensions for debugging
  uint32_t dr0, dr1, dr2, dr3;  // Debug registers
  uint32_t dr6, dr7;             // Debug status/control
};

// Extended debug event types for Zeta
enum zeta_debug_event_type {
  ZETA_MEMORY_LEAK = 100,        // Memory leak detected
  ZETA_DEADLOCK = 101,           // Deadlock detected
  ZETA_EXCEPTION = 102,          // CPU exception
};
...
// Watchpoint tracking
struct Watchpoint {
  BeOS::Address address;
  uint32_t size;
  uint32_t type;  // Read, write, or execute
};

static std::map<int, Watchpoint> g_watchpoints;  // DR register index -> watchpoint

ErrorCode Debug::attachTeam(BeOS::team_id team) {
  DS2LOG(Debug, "attaching to Zeta team: %d", team);

  // Zeta uses enhanced debugger nub with additional capabilities
  return kSuccess;
}

ErrorCode Debug::detachTeam(BeOS::team_id team) {
  DS2LOG(Debug, "detaching from Zeta team: %d", team);
  return kSuccess;
}

ErrorCode Debug::killTeam(BeOS::team_id team) {
  DS2LOG(Debug, "killing Zeta team: %d", team);
  return kSuccess;
}

ErrorCode Debug::suspendThread(BeOS::thread_id thread) {
  DS2LOG(Debug, "suspending Zeta thread: %d", thread);
  return kSuccess;
}

ErrorCode Debug::resumeThread(BeOS::thread_id thread) {
  DS2LOG(Debug, "resuming Zeta thread: %d", thread);
  return kSuccess;
}

ErrorCode Debug::singleStepThread(BeOS::thread_id thread) {
  DS2LOG(Debug, "single-stepping Zeta thread: %d", thread);
...
ErrorCode Debug::setBreakpoint(BeOS::team_id team, BeOS::Address address) {
  DS2LOG(Debug, "setting Zeta breakpoint at 0x%08llx for team %d",
...
ErrorCode Debug::clearBreakpoint(BeOS::team_id team, BeOS::Address address) {
  DS2LOG(Debug, "clearing Zeta breakpoint at 0x%08llx",
...
ErrorCode Debug::setWatchpoint(BeOS::team_id team, BeOS::Address address,
  ...
  DS2LOG(Debug, "setting Zeta watchpoint at 0x%08llx, size=%u, type=%u",
         (unsigned long long)address.value(), size, type);

  // Find free debug register (DR0-DR3)
  int drIndex = -1;
  for (int i = 0; i < 4; i++) {
    if (g_watchpoints.find(i) == g_watchpoints.end()) {
      drIndex = i;
      break;
    }
  }
  ...
  if (drIndex == -1) {
    DS2LOG(Error, "no free debug registers for watchpoint");
    return kErrorInvalidArgument;
  }
  ...
  wp.size = size;
  wp.type = type;
  g_watchpoints[drIndex] = wp;

  DS2LOG(Debug, "configured watchpoint in DR%d", drIndex);

  return kSuccess;
}

ErrorCode Debug::clearWatchpoint(BeOS::team_id team, BeOS::Address address) {
  DS2LOG(Debug, "clearing Zeta watchpoint at 0x%08llx",
         (unsigned long long)address.value());

  // Find and remove watchpoint
  for (auto it = g_watchpoints.begin(); it != g_watchpoints.end(); ++it) {
    if (it->second.address == address) {
      g_watchpoints.erase(it);
      return kSuccess;
    }
  }
...
ErrorCode Debug::readMemory(BeOS::team_id team, BeOS::Address address,
  ...
  DS2LOG(Debug, "reading %zu bytes from 0x%08llx in Zeta team %d",
...
ErrorCode Debug::writeMemory(BeOS::team_id team, BeOS::Address address,
  ...
  DS2LOG(Debug, "writing %zu bytes to 0x%08llx in Zeta team %d",
         size, (unsigned long long)address.value(), team);
...
ErrorCode Debug::readRegisters(BeOS::thread_id thread, void *regs,
  ...
  auto it = g_threadStates.find(thread);
  if (it == g_threadStates.end()) {
    DS2LOG(Error, "thread state not found for Zeta thread %d", thread);
    return kErrorInvalidArgument;
  }
  ...
  // Debug registers (Zeta extension)
...
ErrorCode Debug::getTeamInfo(BeOS::team_id team, BeOS::team_info &info) {
  DS2LOG(Debug, "getting Zeta team info for team %d", team);
  ...
  info.area_count = 0;
  info.flags = 0;
  snprintf(info.name, sizeof(info.name), "Zeta Team %d", team);
...
ErrorCode Debug::getThreadInfo(BeOS::thread_id thread,
  ...
  DS2LOG(Debug, "getting Zeta thread info for thread %d", thread);
  ...
  info.priority = 10;
  snprintf(info.name, sizeof(info.name), "zeta_thread_%d", thread);

  return kSuccess;
}

ErrorCode Debug::getImageInfo(BeOS::area_id image, BeOS::image_info &info) {
  DS2LOG(Debug, "getting Zeta image info for image %d", image);
...
ErrorCode Debug::getAreaInfo(BeOS::area_id area, BeOS::area_info &info) {
  DS2LOG(Debug, "getting Zeta area info for area %d", area);
...
ErrorCode Debug::waitForEvent(BeOS::team_id team, BeOS::debug_event &event,
                              uint64_t timeout_us) {
  DS2LOG(Debug, "waiting for Zeta debug event from team %d", team);
...
ErrorCode Debug::getTeamThreads(BeOS::team_id team,
  ...
  DS2LOG(Debug, "enumerating Zeta threads for team %d", team);
...
ErrorCode Debug::getTeamImages(BeOS::team_id team,
  ...
  DS2LOG(Debug, "enumerating Zeta images for team %d", team);
...
ErrorCode Debug::getTeamAreas(BeOS::team_id team,
  ...
  DS2LOG(Debug, "enumerating Zeta areas for team %d", team);
...
ErrorCode Debug::enableSyscallTrace(BeOS::team_id team, bool enable) {
  DS2LOG(Debug, "%s syscall tracing for Zeta team %d",
         enable ? "enabling" : "disabling", team);

  // Zeta has enhanced syscall tracing with filtering
  return kSuccess;
}
