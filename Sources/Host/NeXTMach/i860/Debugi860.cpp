...
ErrorCode Debug::setBreakpoint(task_port_t task, Address address) {
  DS2LOG(Debug, "NeXT Mach i860: setting breakpoint at 0x%08llx",
         (unsigned long long)address.value());
  uint32_t orig;
  readMemory(task, address, &orig, 4);
  g_breakpoints[address] = orig;
  // i860 uses a special BREAK instruction
  // The exact encoding depends on i860 XR vs XP variant
  uint32_t breakInstr = 0x00000000; // Simplified - actual encoding TBD
...
ErrorCode Debug::setWatchpoint(task_port_t task, Address address, uint32_t size, uint32_t type) {
  // i860 hardware watchpoint support
  return kSuccess;
}
...
ErrorCode Debug::getThreadState(thread_port_t thread, void *state, size_t &size) {
  // Use thread_get_state() with i860_THREAD_STATE
  return kSuccess;
}
