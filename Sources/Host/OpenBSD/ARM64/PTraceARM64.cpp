...
ErrorCode PTrace::readCPUState(ProcessThreadId const &ptid,
  ...
  // OpenBSD ARM64 register layout
  for (int i = 0; i < 30; i++) {
    state.arm64.gp.regs[i] = gprs.r_x[i];
  }
  state.arm64.gp.fp = gprs.r_x[29];
  state.arm64.gp.lr = gprs.r_lr;
  state.arm64.gp.sp = gprs.r_sp;
  state.arm64.gp.pc = gprs.r_pc;
  state.arm64.gp.cpsr = gprs.r_cpsr;
...
ErrorCode PTrace::writeCPUState(ProcessThreadId const &ptid,
  ...
  for (int i = 0; i < 30; i++) {
    gprs.r_x[i] = state.arm64.gp.regs[i];
  }
  gprs.r_x[29] = state.arm64.gp.fp;
  gprs.r_lr = state.arm64.gp.lr;
  gprs.r_sp = state.arm64.gp.sp;
  gprs.r_pc = state.arm64.gp.pc;
  gprs.r_cpsr = state.arm64.gp.cpsr;
