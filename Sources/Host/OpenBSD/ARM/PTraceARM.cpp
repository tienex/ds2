...
ErrorCode PTrace::readCPUState(ProcessThreadId const &ptid,
  ...
  if (::ptrace(PT_GETFPREGS, pid, (caddr_t)&fprs, 0) >= 0) {
    // VFP registers
    for (int i = 0; i < 32; i++) {
      state.arm.vfp.sd[i] = fprs.fpr_vfp_regs[i];
    }
    state.arm.vfp.fpscr = fprs.fpr_vfp_fpscr;
  }
...
ErrorCode PTrace::writeCPUState(ProcessThreadId const &ptid,
  ...
  struct fpreg fprs;
  for (int i = 0; i < 32; i++) {
    fprs.fpr_vfp_regs[i] = state.arm.vfp.sd[i];
  }
  fprs.fpr_vfp_fpscr = state.arm.vfp.fpscr;
