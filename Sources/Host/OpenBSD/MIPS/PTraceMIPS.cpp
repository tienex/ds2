...
ErrorCode PTrace::readCPUState(ProcessThreadId const &ptid,
  ...
  //
  // Copy MIPS general purpose registers (32-bit)
  // OpenBSD MIPS struct reg layout
  //
  for (int i = 0; i < 32; i++) {
  ...
  if (::ptrace(PT_GETFPREGS, pid, (caddr_t)&fpregs, 0) >= 0) {
    // MIPS32 uses 32 single-precision FPU registers
    for (int i = 0; i < 32; i++) {
      state.fpu.sng[i] = fpregs.r_regs[i];
    }
...
ErrorCode PTrace::writeCPUState(ProcessThreadId const &ptid,
  ...
  struct fpreg fpregs;
  for (int i = 0; i < 32; i++) {
    fpregs.r_regs[i] = state.fpu.sng[i];
  }
